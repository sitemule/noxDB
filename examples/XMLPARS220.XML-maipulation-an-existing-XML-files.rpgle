      ...
      /include qrpgleRef,noxdb
        // first we run the paser on a minimal string to build the XML tree with the root name "root
          pXml = xml_ParseFile ('/noxdb/IceCap13.xml' :'syntax=LOOSE');
          If xml_Error(pXml) ;
          ...
          pElem = xml_locate(pXml:'/icecap/panels');
          If pElem <> *NULL;
             pPanel = xml_elementAdd(pElem:xml_LAST_CHILD:tag:value);
          EndIf;

          xml_writeStmf(pXml : '/noxdb/IceCap13out.xml': 1208);
          xml_delete(pXml);
          *inlr = *on;
