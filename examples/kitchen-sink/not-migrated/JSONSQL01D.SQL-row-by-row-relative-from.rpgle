...
// ------------------------------------------------------------- 
   ...
   // Need some statisics ?
   rows = json_sqlRows(sqlhnd);    // Note this will produce a "select count(*)" and can have extra overhead
   cols = json_sqlColumns(sqlhnd); // Number of columns in the resultset.
   
   // Now iterate on each row in the resultset
   // NOTE!! Start from row 10 and read ahead. Can be used as long no "start" and "limit" is provided at the sqlOpen
