   ...
   if ( pSQL->rc == SQL_SUCCESS) {
      ...
      for (i = 0; i < pSQL->nresultcols; i++) {
         ...
         if (pCol->outlen ==  SQL_NULL_DATA) {
            ...
            SQLGetData(pSQL->pstmt->hstmt, i+1, SQL_C_CHAR, buf , sizeof(buf), &buflen);
...
/* ------------------------------------------------------------- */
LGL jx_sqlUpsert (BOOL update, PUCHAR table  , PJXNODE pRowP , PUCHAR whereP, PJXNODE pSqlParms)
{
   ...
   if (update) {
      buildUpdate (pSQL->pstmt->hstmt, sqlTempStmt , table, pRow , where);
   } else {
      buildInsert (pSQL->pstmt->hstmt, sqlTempStmt , table, pRow , where);
   }
   ...
   for (i=1; pNode; i++) {
      ...
      if (pNode->type == ARRAY ||  pNode->type == OBJECT) {
         int l;
         value = valArr[valArrIx++] = memAlloc(1000000);
         l = jx_AsJsonTextMem (pNode , value);
         value [l] = '\0';
      } else {
         value = jx_GetNodeValuePtr  (pNode , NULL);
      }
...
LGL jx_sqlUpdate (PUCHAR table  , PJXNODE pRow , PUCHAR whereP, PJXNODE pSqlParmsP  )
{
   ...
   return jx_sqlUpsert  (true , table  , pRow , where, pSqlParms);
}
LGL jx_sqlInsert (PUCHAR table  , PJXNODE pRow , PUCHAR whereP, PJXNODE pSqlParmsP  )
{
   ...
   PUCHAR  where    =  (pParms->OpDescList->NbrOfParms >= 3) ? whereP : "";
   PJXNODE pSqlParms = (pParms->OpDescList->NbrOfParms >= 4) ? pSqlParmsP : NULL;
   return jx_sqlUpsert  (false , table  , pRow , where , pSqlParms);
}
