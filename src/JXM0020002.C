...
//   PJXSQL pSQL = jx_sqlNewStatement (pParms->OpDescList->NbrOfParms >= 2 ? pSqlParms  :NULL);
   ...
   */
   ...
   for (i = 0; i < pSQL->nresultcols; i++) {
      ...
      // get display label  for column ( fails on 5.4
      rc = SQLColAttribute (pSQL->pstmt->hstmt,i+1,SQL_DESC_LABEL,label  , 128 , &len , &descLen);
      if (rc != SQL_SUCCESS ) {
        // check_error (pSQL); will return erroro "HY009"
