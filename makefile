...
C_FLAGS=OPTIMIZE(10) ENUM(*INT) TERASPACE(*YES) STGMDL(*INHERIT) SYSIFCOPT(*IFSIO) DBGVIEW(*ALL) OPTION(*EVENTF) OUTPUT(*PRINT) TGTRLS($(TARGET_RELEASE))
#DEBUG
#C_FLAGS=OPTIMIZE(10) ENUM(*INT) TERASPACE(*YES) STGMDL(*INHERIT) SYSIFCOPT(*IFSIO) OPTION(*EVENTF *EXPMAC *SHOWINC) OUTPUT(*PRINT) TGTRLS($(TARGET_RELEASE))
...
linkcopy:
	...
	-system "CPYTOSTMF FROMMBR('/QSYS.LIB/QOAR.LIB/H.file/QRNTYPES.MBR') TOSTMF('./ext/include/qoar/qrntypes.h') STMFCCSID(1252)"
	-system "CPYTOSTMF FROMMBR('/QSYS.LIB/QOAR.LIB/H.file/QRNDTAGEN.MBR') TOSTMF('./ext/include/qoar/qrndtagen.h') STMFCCSID(1252)"
	-system "CPYTOSTMF FROMMBR('/QSYS.LIB/QOAR.LIB/H.file/QRNDTAINTO.MBR') TOSTMF('./ext/include/qoar/qrndtainto.h') STMFCCSID(1252)"
