...
PJXNODE jx_traceNode (PUCHAR text, PJXNODE pNode)
{
  ...
  if (debugger == 1) {
     sprintf(filename, "/jsonxml/dump/jsontrace-%05.5d.json" , i ++);
     jx_WriteJsonStmf (pNode, filename , 1208, OFF, NULL);
  } else if (debugger == 2) {
     UCHAR temp [65536];
     int l = jx_AsJsonTextMem (pNode , temp);
     temp [l] = 0;
     puts (text);
     puts (temp);
     puts ("\n");
...
PJXNODE jx_ArraySort(PJXNODE pNode, PUCHAR fieldsP)
{
   ...
   for(kix=0 ; kix < 32 ; kix ++) {
      if (*subword(keys[kix], fields, kix , ",") == '\0') break;
   }
   ...
   do {
      ...
      while (pNode2) {
         for(kx = 0; kx < kix ; kx++)  {
            ...
            comp = strcmp (pCompNode1 ? pCompNode1->Value:"" , pCompNode2 ? pCompNode2->Value:"");
            switch (comp) {
               case 0: break;
               case 1: {
                  bubles = true;
...
// ---------------------------------------------------------------------------
void indent (PJWRITE pJwrite) {
   static UCHAR  crlf []  =  {0x0d , 0x0a , 0x00};
   SHORT i;
   if (pJwrite->doTrim) return;

   fputs (crlf , pJwrite->outFile);
   for (i=0;i<(pJwrite->level*2);i++) {
     fputc (0x20 , pJwrite->outFile);
   }
}
/* ---------------------------------------------------------------------------
   --------------------------------------------------------------------------- */
...
/* --------------------------------------------------------------------------- */
static void jWriter (PJWRITE pJwrite, PJXNODE pNode , PUCHAR Ctlstr , ... )
{
   va_list arg_ptr;
   SHORT len;
   static BOOL nextOnSame;

   // Build a temp string with the formated data  */
   va_start(arg_ptr, Ctlstr);
   len = vsprintf(pJwrite->buf, Ctlstr, arg_ptr);
   va_end(arg_ptr);
   ...
   if (!pJwrite->doTrim) {
     if (! pJwrite->isDefaultFormatDetected ) {
        if (pNode->pNodeChildHead && pNode->pNodeChildTail) {
           if (pNode->pNodeChildHead->lineNo == pNode->pNodeChildTail->lineNo) {
              pJwrite->useDefaultFormat = TRUE;
           }
        }
        pJwrite->isDefaultFormatDetected = TRUE;
     }
   }
   ...
   if (pJwrite->useDefaultFormat) {
     ...
     if (len > 2 && pJwrite->buf[len-2] == ':') {
       indent(pJwrite);
       nextOnSame = TRUE;
       goto dowrite;
     }
     if (len > 2 || *pJwrite->buf == CurEnd || *pJwrite->buf == BraEnd) {
       indent(pJwrite);
     }
     goto dowrite;
   }

   if ((pJwrite->lineNo != pNode->lineNo && pNode->lineNo != 0)
   ||  (pJwrite->level < pJwrite->prevLevel)
   ||  (pNode->lineNo == 0 && len > 2 && pJwrite->buf[len-2] == ':')) { // TODO the ,is not the best way to se breaks
     if (pJwrite->lineNo < pNode->lineNo) pJwrite->lineNo = pNode->lineNo;
     pJwrite->prevLevel = pJwrite->level;
     indent(pJwrite);
     goto dowrite;
   }

   dowrite:
   iconvWrite( pJwrite->outFile , &pJwrite->iconv, pJwrite->buf, FALSE);
}
/* --------------------------------------------------------------------------- */
static void  jx_AsJsonTextList (PJXNODE pNode, PJWRITE pJwrite)
{
   PJXNODE  p;
   BOOL  comma = FALSE;
   SHORT maxNameLen =0;

   // Find length of longest name, so we can allign
   if (!pJwrite->doTrim) {
     int maxLin=0, minLin = 9999999 ;
     for (p = pNode; p ; p=p->pNodeSibling) {
       if (p->Name) {
          SHORT l = strlen(p->Name);
          if (l > maxNameLen) maxNameLen = l;
          if (p->lineNo < minLin) minLin = p->lineNo;
          if (p->lineNo > maxLin) maxLin = p->lineNo;
       }
     }
     if (minLin == maxLin) maxNameLen = 0; // No extra space if all on same line
   }

   while (pNode) {

      if (pNode->type != UNKNOWN) {
        if (comma) {
          jWriter (pJwrite, pNode, ",");
        } else if (pNode->pNodeSibling && !pJwrite->doTrim) { // Indent if we have a sibling
          jWriter (pJwrite, pNode, " ");
        }
      }

      if (pNode->Name && *pNode->Name
      &&  pNode->pNodeParent && pNode->pNodeParent->type != ARRAY
      &&  pNode->type != UNKNOWN
      &&  pJwrite->level > 0) {
         if (pJwrite->doTrim) {
           jWriter (pJwrite, pNode, "%c%s%c:", Quot, pNode->Name, Quot);
         } else {
           int padlen = maxNameLen - strlen(pNode->Name);
           if (padlen <0) padlen = 0;
           jWriter (pJwrite, pNode, "%c%s%c%*s: ", Quot, pNode->Name, Quot, padlen, "");
         }
      }

      switch (pNode->type) {
        case OBJECT  : jWriter (pJwrite, pNode, "%c", CurBeg); break;
        case ARRAY   : jWriter (pJwrite, pNode, "%c", BraBeg); break;
        case UNKNOWN :                             ; break;
        default      :
          if (pNode->Value && pNode->Value[0] > '\0') {
            if (pNode->isLiteral) {
              jWriter (pJwrite, pNode,  "%s", pNode->Value);
            } else {
              PUCHAR w1 = memAlloc(512000);
              jWriter (pJwrite, pNode,  "%c%s%c", Quot, jx_EncodeJson(w1 , pNode->Value), Quot);
              memFree (&w1);
            }
          } else if (pNode->Value && ! pNode->isLiteral)  {
            jWriter (pJwrite, pNode, "%c%c", Quot, Quot);
          } else if (pNode->Name) {   // Only for key/value pairs. TODO!! Fix: Objects parent might have vlaue parent
            jWriter (pJwrite, pNode, "null");
          }
      }

      if (pNode->pNodeChildHead) {
        pJwrite->level ++;
        jx_AsJsonTextList (pNode->pNodeChildHead , pJwrite);
        pJwrite->level --;
      }

      switch (pNode->type) {
        case OBJECT  : jWriter (pJwrite, pNode, "%c", CurEnd); break;
        case ARRAY   : jWriter (pJwrite, pNode, "%c", BraEnd); break;
      }

      pNode = (pJwrite->level == 0) ? NULL :pNode->pNodeSibling;
//      if (pNode && pNode->type != UNKNOWN) jWriter (pJwrite, pNode, ",");
      comma = TRUE;
   }
...
LONG jx_AsJsonTextMem (PJXNODE pNode, PUCHAR buf)
{
   ...
   jwrite.buf = buf;
   if ( pNode == NULL) return 0;
   jx_AsJsonTextList (pNode , &jwrite);
   return  jwrite.buf -  buf;
...
{
   VARCHAR  res;
   res.Length = jx_AsJsonTextMem ( pNode ,  res.String);
   return res;
}
/* ---------------------------------------------------------------------------
   --------------------------------------------------------------------------- */
void jx_WriteJsonStmf (PJXNODE pNode, PUCHAR FileName, int Ccsid, LGL trimOut, PJXNODE options)
{
   ...
   jx_AsJsonTextList (pNode , pJwrite);
...
// ---------------------------------------------------------------------------
void  detectEncoding(PJXCOM pJxCom, PUCHAR pIn , BOOL overrideCharset)
{
  UCHAR  e2aTbl [256];
  UCHAR  buf [128];
  PUCHAR p;
  ...
  for (i=0; ! done; i++, p++) {
   ...
   #pragma convert(0)
      ...
      default:
        if (*p == BraBeg || *p == CurBeg) { // For other codepages than 277
           pJxCom->isJson = TRUE;
           done = TRUE;
           break;
        }
  ...
  if (overrideCharset) {
    ...
    // Assume 1208 for any ascii JSON
    if (isAscii && InputCcsid == 0) {
       InputCcsid = 1208;
    }
    ...
    InputCcsid = 0;        // EBCDIC -> the use current ccsid which:0
    if (pJxCom->File) {
      if (lstat(pJxCom->FileName, &statbuf)) {
        jx_SetMessage( "File %s not open", pJxCom->FileName);
        pJxCom->State = XML_EXIT_ERROR;
        return;
      }

      if (statbuf.st_ccsid < 900) { // has to be somekind of ebcdic or PC codepage
        InputCcsid = statbuf.st_ccsid;
      }
    }
...
// ---------------------------------------------------------------------------
// unlink a node freom the tree and returns it as a new root
// ---------------------------------------------------------------------------
PJXNODE jx_NodeUnlink  (PJXNODE  pNode)
{
  PJXNODE pNewRoot, pPrevNode, pParent = NULL;

  if (pNode == NULL) return NULL;

  // if I am a root node, then look no further
  if (pNode->pNodeParent == NULL) return pNode;
  ...
  if (pParent && pParent->pNodeChildTail == pNode) {
     pParent->pNodeChildTail = pNode;
  }
  return pNode;
}
// ---------------------------------------------------------------------------
...
PJXNODE jx_NodeMoveInto (PJXNODE  pDest, PUCHAR name , PJXNODE pSource)
{
   ...
   // }
      ...
      if ( pDest->type != OBJECT && pDest->type != ARRAY ) {
         pDest->type = OBJECT;
      }
      memFree(&(pDest->Value));
...
void jx_NodeMoveAndReplace (PJXNODE  pDest, PJXNODE pSource)
{
   ...
   memcpy(&oldDest , pDest , sizeof(JXNODE));

   // Can not use jx_FreeSiblings on pDest - we need to physically be at the same position in memory
   freeAttrList (pDest->pAttrList);

   jx_FreeChildren  (pDest);

   memFree(&pDest->Value);

   pSource = jx_NodeUnlink(pSource); // Now I am my own root

   memcpy(pDest , pSource , sizeof(JXNODE));
   pDest->pNodeSibling =  oldDest.pNodeSibling;
   pDest->pNodeParent  =  oldDest.pNodeParent;
   pDest->Name        =  oldDest.Name;

   // Unlink the source node from the original source tree so it will not be deleted by "close"
   if (pSource->pNodeParent == NULL) { // Only if i'm not the root
     pSource->pNodeChildHead = NULL;   // This will cause the tree deltion in "close" to be disabled
     pSource->pAttrList     = NULL;   // This will cause the attribute deltion in "close" to be disabled
   } else {
     pNode  = previousSibling(pSource);
     if (pNode) {
        pNode->pNodeSibling = pSource->pNodeSibling;
     } else {
        pSource->pNodeParent->pNodeChildHead = pSource->pNodeSibling;
     }
     if (pSource->pNodeParent->pNodeChildTail == pSource) {
        pSource->pNodeParent->pNodeChildTail = pSource;
     }
     memFree (&pSource);
   }
}
...
void jx_NodeSetAsPointer (PJXNODE pNode , PUCHAR Value)
{
   ...
   if (pNode->type != POINTER_VALUE ) {
       memFree(&pNode->Value );  // If i was a value - - drop it
...
void jx_NodeDelete(PJXNODE pNode)
{
   ...
   if (pNode->pNodeParent) {
      pTemp  = previousSibling(pNode);
      if (pTemp) {
         pTemp->pNodeSibling = pNode->pNodeSibling;
      } else {
         pNode->pNodeParent->pNodeChildHead = pNode->pNodeSibling;
      }
   }
   jx_NodeFree(pNode);
}
...
PJXNODE jx_ParseString(PUCHAR Buf, PUCHAR pOptions)
{
   ...
   detectEncoding(pJxCom, Buf, false);
   if (pJxCom->UseIconv) {
...
{
   PUCHAR  Name = * ppName;
   PJXNODE pNodeTemp =pRootNode->pNodeChildHead;
   PJXNODE pNode;
   ...
   pEnd = findchr(compVal , delimiters , sizeof(delimiters));
   compLen = pEnd - compVal;
   *ppName =  *ppName + (pEnd - Name);

   while (pNodeTemp) {
      pNode = jx_GetNode  (pNodeTemp, keyName);
...
/* -------------------------------------------------------------
   ...
   ------------------------------------------------------------- */
static VOID jx_NodeAddAttributeValue  (PJXNODE pNode , PUCHAR AttrName, PUCHAR Value)
{
...
PUCHAR jx_GetValuePtr (PJXNODE pNodeRoot, PUCHAR Name, PUCHAR Default)
{
   ...
   if (pAtrKey) {
      ...
      if (pNode->Value == NULL) return dft;
      return pNode->Value;
   }
}
/* --------------------------------------------------------------------------- */
...
void jx_CopyValueByNameVC (PVARCHAR pRes, PJXNODE pNodeRoot, PUCHAR Name, PUCHAR Default , BOOL joinString)
{
   ...
   if (pAtrKey) {
      ...
      if (pRes->Length == 0) { // No data found when joining arrays as string - Now serialize it as usual
         pRes->Length  = jx_AsJsonTextMem (pNode , pRes->String);
      }
...
PJXNODE jx_CreateSubNodes  (PJXNODE pNodeRoot , PUCHAR Path )
{
   ...
   do {
      ...
      if (*pName) {
        ...
        if (pNodeTemp == NULL) {
           memFree(&pParentNode->Value); // Can not have values if we have childrens
           // When i have children, then i must be an object or an array
           if (pParentNode->type == VALUE) {
        ...
        if (isNewArray) {
           memFree(&pNodeTemp->Value); // Can not have values if we have childrens
...
PJXNODE  jx_SetValueByName (PJXNODE pNodeRoot, PUCHAR Name, PUCHAR Value, NODETYPE typePP)
{
   ...
   // Backwards compatible - remve this
   // if (type == CLONE_OLD) type = CLONE;


   // jx_Debug ( "Before" , pNodeRoot);
   ...
   if (pParms->OpDescList->NbrOfParms >= 4) {
      ...
      /****  Done now when the values signature i an object already
      if ( type == OBJMOVE) {
         jx_NodeMoveAndReplace (pParentNode , (PJXNODE) Value);
         return jx_traceNode("Move and replace" ,pParentNode);
      }
      if ( type == CLONE) {
         jx_NodeCopy (pParentNode , (PJXNODE) Value, RL_LAST_CHILD);
         return jx_traceNode("Clone" , pParentNode);
      }
      *****/
...
/* -------------------------------------------------------------
   Set BOOL by name
   ------------------------------------------------------------- */
PJXNODE  jx_SetPtrByName (PJXNODE pNode, PUCHAR Name, PUCHAR Value)
{
   return jx_SetValueByName(pNode , Name , Value , POINTER_VALUE );
}
/* -------------------------------------------------------------
   RPG Wrappers
   ------------------------------------------------------------- */
